...
class AttendanceService {
  ...
  Future<void> checkInOrCheckOut(bool isCheckIn, String deviceId,
  ...
  Future<void> checkInOrCheckOutOffsite(
      bool isCheckIn, Map<String, dynamic> attendanceData) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token');
    ...
    try {
      ...
      if (response.statusCode == 403) {
        ...
        throw Exception(
            'Failed to submit attendance. Error: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Error during the API call: $e');
    }
