...
class _ProjectManagementPageState extends State<ProjectManagementPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _initializeRecorder();
    _fetchAssignments();  // Fetch assignments and extract project ID
  }
  ...
  Future<void> _fetchAssignments() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${ProjectManagementPage.baseUrl}/api/work-tracking/ass/assignments'),
        headers: {
          ...
          setState(() {
            ...
            _tasks.addAll(
              (data['results'] as List).map((item) {
                return {
                  'proj_id': item['proj_id'] ?? 'Unknown', // Default value
                  'title': item['title'] ?? 'No title',    // Default value
                  'description': item['description'] ?? '', // Default value
                  'status': item['status'] ?? 'Unknown',
                  'start_date': item['created_at'] ?? '',
                  'due_date': item['updated_at'] ?? '',
                  'employees_id': item['employees_id'] ?? 'N/A',
                  'employee_name': item['employee_name'] ?? 'Unknown',
                  'total_notifications': item['total_notifications'] ?? '0',
                };
              }).toList(),
            );
            _projID = _tasks.first['proj_id'] ?? 'Unknown';
            _shortenedProjectId = _projID.length > 8 ? '${_projID.substring(0, 8)}...' : _projID;
          });
        } else {
          _showErrorDialog('No assignments found.');
        }
      } else {
        _showErrorDialog('Failed to load assignments. Status code: ${response.statusCode}');
      }
    } catch (e) {
      _showErrorDialog('Error fetching assignments: $e');
    }
  }
              ...
              children: [
                ...
                Column(
                  children: task['images'] != null
                      ? (task['images'] as List<dynamic>).map<Widget>((image) {
  ...
  Widget _buildTaskCard(Map<String, dynamic> task, int index) {
    ...
    final startDate = DateTime.tryParse(task['start_date']) ?? DateTime.now();
    ...
    return GestureDetector(
      ...
      child: Card(
        shape: RoundedRectangleBorder(
          ...
          side: BorderSide(color: progressColors[task['status']] ?? Colors.grey),
        ),
        elevation: 5,
        margin: const EdgeInsets.symmetric(vertical: 8.0),
        child: Padding(
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(
                    task['status'] ?? 'Unknown',
                    style: TextStyle(color: progressColors[task['status']] ?? Colors.grey, fontWeight: FontWeight.bold),
                  ),
                ],
              ),
